...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    qualified_paragraphs = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(qualified_paragraphs):
        return ''
    else:
        return qualified_paragraphs[k]
    # END PROBLEM 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    elif all([diff_function(typed_word, word, limit) > limit for word in word_list]):
        return typed_word
    else:
...
def minimum_mewtations(typed, source, limit):
    ...
    """
    length_diff = abs(len(typed) - len(source))
    if length_diff > limit or (not typed or not source): # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return length_diff
    elif limit < 0:
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for timestamp in tpp:
        times += [[timestamp[i + 1] - timestamp[i] for i in range(len(timestamp) - 1)]]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if get_time(times, player_index, word_index) < get_time(times, fastest_player, word_index):
                fastest_player = player_index
        result[fastest_player] += [words[word_index]]
    return result
    # END PROBLEM 10
